...
def login(body: LoginIn, db: Session = Depends(get_db)):
    user = db.scalar(select(User).where(func.lower(User.user_name) == body.userName.replace(" ", "").lower()))
    if not user or not verify_password(body.password.replace(" ", ""), user.password_hash):
...
def create_sale(body: SaleCreate, db: Session = Depends(get_db), user: User = Depends(current_user)):
    if not body.items: raise HTTPException(400, "Sale must contain at least one item")
    if body.status not in {"pending","paid","not_paid","cancelled"}: raise HTTPException(400, "Invalid sale status")
    if user.role != "admin" and body.cashier != user.full_name: body.cashier = user.full_name
    now = datetime.now(timezone.utc)
    if not body.receipt_no:
        counter = db.scalar(select(ReceiptCounter).where(ReceiptCounter.id == 1).with_for_update())
        if not counter:
            counter = ReceiptCounter(id=1, next_no=1001); db.add(counter); db.flush()
        body.receipt_no = str(counter.next_no)
        counter.next_no += 1
    rows=[SalesHistory(receipt_no=body.receipt_no,date=now,pmt_type=body.pmt_type,customer=body.customer,cashier=body.cashier,prod=i.product,price=i.price,qty=i.qty,total=i.total,status=body.status) for i in body.items]
...
def sales(page: int = Query(1, ge=1), page_size: int = Query(50, ge=1, le=200), receipt_no: str | None = None, status: str | None = None, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    if receipt_no:
        base=base.where(SalesHistory.receipt_no==receipt_no); count_q=count_q.where(SalesHistory.receipt_no==receipt_no)
    if status:
        base=base.where(SalesHistory.status==status); count_q=count_q.where(SalesHistory.status==status)
...
def sale(receipt_no: str, db: Session = Depends(get_db), user: User = Depends(current_user)):
    ...
    if not rows: raise HTTPException(404,"Sale not found")
    return {"receipt_no":receipt_no,"date":rows[0].date,"pmt_type":rows[0].pmt_type,"customer":rows[0].customer,"cashier":rows[0].cashier,"status":rows[0].status,"items":[{"product":r.prod,"price":r.price,"qty":r.qty,"total":r.total} for r in rows],"total":sum(r.total for r in rows)}
...
def sale_status(receipt_no: str, body: SaleStatusUpdate, db: Session = Depends(get_db), user: User = Depends(admin_only)):
    if body.status not in {"pending","paid","not_paid","cancelled"}: raise HTTPException(400,"Invalid status")
    result=db.execute(update(SalesHistory).where(SalesHistory.receipt_no==receipt_no).values(status=body.status))
    if result.rowcount == 0: raise HTTPException(404,"Sale not found")
    db.commit(); return {"receipt_no":receipt_no,"status":body.status}

@app.get("/summary")
def summary(period: str = Query("weekly", pattern="^(weekly|monthly|yearly)$"), db: Session = Depends(get_db), user: User = Depends(admin_only)):
    now=datetime.now(timezone.utc)
    if period == "weekly":
        start = now - timedelta(days=7); bucket = "day"
    elif period == "monthly":
        start = now - timedelta(days=30); bucket = "day"
    else:
        start = now - timedelta(days=365); bucket = "month"
    bucket_expr = func.date_trunc(bucket, SalesHistory.date)
    rows = db.execute(select(bucket_expr.label("bucket"), func.sum(SalesHistory.total).label("value")).where(SalesHistory.date >= start, SalesHistory.status != "cancelled").group_by(bucket_expr).order_by(bucket_expr)).all()
...
def add_user(body: UserCreate, db: Session = Depends(get_db), user: User = Depends(admin_only)):
    if db.get(User, body.userId): raise HTTPException(409,"User ID already exists")
    if db.scalar(select(User).where(func.lower(User.user_name)==body.userName.lower())): raise HTTPException(409,"Username already exists")
    role="admin" if body.role=="admin" or body.userName.lower().startswith("adm") else "cashier"
    ...
    db.add(u); db.commit(); db.refresh(u); return user_out(u)
...
def edit_user(user_id: str, body: UserUpdate, db: Session = Depends(get_db), user: User = Depends(admin_only)):
    ...
    if "userName" in data and data["userName"] != u.user_name:
        if db.scalar(select(User).where(func.lower(User.user_name)==data["userName"].lower())): raise HTTPException(409,"Username already exists")
        u.user_name=data["userName"]
    if "fullName" in data: u.full_name=data["fullName"]
    if "role" in data and data["role"] in {"admin","cashier"}: u.role=data["role"]
